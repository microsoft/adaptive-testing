...
export default class Tree extends React.Component {
  constructor(props) {
    ...
    // console.log("this.props.id", this.props.id, this.props.comm)
  ...
  render() {
    const open = this.state.open || this.state.selected || this.state.child_selected;
    let icon;
    ...
    return <div style={{marginLeft: (parseFloat(this.props.level) * 4) + "px"}}>
      <div style={{display: "flex", alignItems: "stretch"}}>
        <div style={{width: "4px", background: this.state.selected ? "rgb(9, 105, 218)" : "none", borderRadius: "2px", marginRight: "5px", display: "flex", alignItems: "center"}}>
          {/* <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 504.35" width="14" style={{marginLeft: "-18px", opacity: this.state.selected ? 1 : 0}}>
          ...
          {/* <svg viewBox="0 0 276.99 507.01" width="8" style={{marginLeft: "-18px", opacity: this.state.selected ? 1 : 0}}>
          ...
          <FontAwesomeIcon onClick={this.toggleOpen} icon={faSquare} style={{fontSize: "14px", color: "#000", opacity: this.state.selected ? 0.2 : 0, display: "inline-block", marginLeft: "-20px"}} />
        </div>
        <div style={{borderRadius: "7px 7px 7px 7px", paddingLeft : "4px", flexGrow: 1, paddingRight : "4px", background: this.state.selected ? "rgba(1, 1, 1, 0.04)" : "none", cursor: "pointer"}} 
          ...
          {this.state.children && open && this.state.children.map((child_id, index) => {
            return <Tree key={index} level={parseFloat(this.props.level)} comm={this.props.comm} onClick={this.props.onClick} id={child_id} show_label={true} />;
          })}
        </div>
      </div>
      
      {this.state.selected && false && <div style={{marginLeft: ((parseFloat(this.props.level)+1) * 4 + 4 + 4 + 6) + "px", padding: "6px", paddingLeft: "7px", paddingTop: "4px"}}>
