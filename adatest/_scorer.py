...
class GeneratorScorer(Scorer):
    ...
    def __init__(self, model, topk=1, output_names=None, method="dirichlet", dirichlet_concentration=10, **kwargs):
        """ Create a new scorer given a model that returns a probability vector for each input string.
...
class ImageScorer(Scorer):
    ...
    def __init__(self, model, topk=1, output_names=None, method="dirichlet", dirichlet_concentration=10, wait_seconds = 5):
        ...
        # Set used to prevent input from being queued multiple times
        self.pending_inputs = set()
        self.pending_inputs_lock = threading.Lock()
        self.worker = threading.Thread(target=self._model_worker, args=(wait_seconds,), daemon=True)
        self.worker.start()
